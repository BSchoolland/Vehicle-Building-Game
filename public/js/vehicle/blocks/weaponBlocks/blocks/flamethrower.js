...
class flameThrower extends Block {
  constructor(x, y, contraption) {
    ...
    this.attackFlameDuration = 15; // the duration of fire this gives to other blocks
    this.attackFlameDamage = 30; // the damage per second this block's flame does to other blocks
    this.flames = [];
  }
  makeBodies() {
  ...
  makeConstraints() {
    ...
    // constrain the joint and the nozzle, allowing rotation
    this.constraints.push(
      Matter.Constraint.create({
        bodyA: this.bodies[1], // the nozzle
        bodyB: this.bodies[2], // the joint
        pointA: { x: 10, y: 0 },
        pointB: { x: 0, y: 0 },
        stiffness: 1,
  ...
  createFlame(deltaTime) {
    ...
    const inaccuracyVector = Matter.Vector.rotate(normalizedDirection, inaccuracy);
    const force = Matter.Vector.mult(inaccuracyVector, (-this.thrust * deltaTime) / 1000);
    Matter.Body.applyForce(flame, this.bodies[1].position, force);
    // apply a slight upward force to the flame
    Matter.Body.applyForce(
        flame,
        flame.position,
        { x: 0, y: -0.01 }
  ...
  hit(flame, otherBody) {
    ...
    if (otherBody.block === "ground") return; // if the other block can't be set on fire, don't hit it
